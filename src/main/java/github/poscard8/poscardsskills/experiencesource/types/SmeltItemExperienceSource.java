...
// same code as CraftItemExperienceSource

public class SmeltItemExperienceSource extends SimpleExperienceSource<ItemStack> {

    private static final Map<Player, List<ItemStack>> WAITING_ITEMS_MAP = new HashMap<>();


    SmeltItemExperienceSource(Skill skill, int xp, Predicate<ItemStack> predicate) { super(skill, xp, predicate); }

    public static SmeltItemExperienceSource fromJsonObject(ResourceLocation location, JsonObject jsonObject) {
        ...
        if (jsonObject.has("item")) {
            ...
            if (itemArg.equals("ALL")) return new SmeltItemExperienceSource(skill, xp, ExperienceSourcePredicates.ALWAYS_TRUE_ITEM_STACK);
        ...
        return new SmeltItemExperienceSource(skill, xp, predicate);
    }
    ...
    public static void handlePlayer(Player player) {
        ...
        for (ItemStack stack : getWaitingItems(player)) {
            ...
            for (SmeltItemExperienceSource xpSource : ExperienceSource.filterBy(SmeltItemExperienceSource.class)) xpSource.applyIfMeetsConditions(player, stack, stack.getCount() / 2);
        }
