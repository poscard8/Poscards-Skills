...
import static github.poscard8.poscardsskills.util.client.model.BakedQuadHelper.crop;

@OnlyIn(Dist.CLIENT)
@MethodsReturnNonnullByDefault
public class LayeredBlockModel extends ShiftedTextureModel
{
    ...
    public LayeredBlockModel(BakedModel baked) { super(baked); }

    @Override
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, ModelData data, List<BakedQuad> original)
    {
        ...
        if (side.getAxis().isVertical())
        {
            for (BakedQuad quad : original)
            {
                ...
                ResourceLocation textureLoc = quad.getSprite().contents().name();
    ...
    public ModelData getModelData(BlockAndTintGetter level, BlockPos position, BlockState state)
    {
        ...
        builder
                .with(NORTH, checkBlocks(level, state.getBlock(), position.north()))
                .with(SOUTH, checkBlocks(level, state.getBlock(), position.south()))
                .with(EAST, checkBlocks(level, state.getBlock(), position.east()))
                .with(WEST, checkBlocks(level, state.getBlock(), position.west()))
                .with(NORTH_EAST, checkBlocks(level, state.getBlock(), position.north(), position.north().east(), position.east()))
                .with(NORTH_WEST, checkBlocks(level, state.getBlock(), position.north(), position.north().west(), position.west()))
                .with(SOUTH_EAST, checkBlocks(level, state.getBlock(), position.south(), position.south().east(), position.east()))
                .with(SOUTH_WEST, checkBlocks(level, state.getBlock(), position.south(), position.south().west(), position.west()));

        return builder.build();
    }

    protected boolean checkBlocks(BlockAndTintGetter level, Block block, BlockPos... positions)
    {
        for (BlockPos position : positions) {

            if (!level.getBlockState(position).is(block)) return false;
        }
        return true;
    }
