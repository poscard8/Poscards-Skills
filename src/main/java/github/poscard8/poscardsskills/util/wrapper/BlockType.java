...
public enum BlockType {
    ...
    private final String textureFormat;
    ...
    final BiConsumer<BlockLoot, BlockWrapper> lootTableFunction;
    ...
    BlockType(String nameFormat, String textureFormat, Function<BlockSet, Supplier<Block>> registerFunction,
              BiConsumer<BlockStateProvider, BlockWrapper> blockModelFunction, BiConsumer<ItemModelProvider, BlockWrapper> itemModelFunction, BiConsumer<BlockLoot, BlockWrapper> lootTableFunction) {
    ...
    private static Supplier<Block> stairsRegister(BlockSet blockSet) { return () -> new StairBlock(Blocks.STONE.defaultBlockState(), blockSet.getProperties()); }

    private static Supplier<Block> slabRegister(BlockSet blockSet) { return () -> new SlabBlock(blockSet.getProperties()); }

    private static Supplier<Block> wallRegister(BlockSet blockSet) { return () -> new WallBlock(blockSet.getProperties()); }

    private static Supplier<Block> pillarRegister(BlockSet blockSet) { return () -> new PillarBlock(blockSet.getProperties()); }

    private static Supplier<Block> layeredRegister(BlockSet blockSet) { return () -> new LayeredBlock(blockSet.getProperties()); }

    private static Supplier<Block> shinyRegister(BlockSet blockSet) { return () -> new ShinyBlock(blockSet.getProperties()); }

    private static void blockModel(BlockStateProvider provider, BlockWrapper wrapper) { provider.simpleBlock(wrapper.get()); }

    private static void stairModel(BlockStateProvider provider, BlockWrapper wrapper) { provider.stairsBlock((StairBlock) wrapper.get(), wrapper.textureLocation()); }

    private static void slabModel(BlockStateProvider provider, BlockWrapper wrapper) { provider.slabBlock((SlabBlock) wrapper.get(), wrapper.textureLocation(), wrapper.textureLocation()); }

    private static void wallModel(BlockStateProvider provider, BlockWrapper wrapper) { provider.wallBlock((WallBlock) wrapper.get(), wrapper.textureLocation()); }

    private static void chiseledModel(BlockStateProvider provider, BlockWrapper wrapper) {
    ...
    private static void pillarModel(BlockStateProvider provider, BlockWrapper wrapper) {
    ...
    private static void layeredModel(BlockStateProvider provider, BlockWrapper wrapper) {
    ...
    private static void shinyModel(BlockStateProvider provider, BlockWrapper wrapper) {
    ...
    private static void roughModel(BlockStateProvider provider, BlockWrapper wrapper) {
    ...
    private static void blockItemModel(ItemModelProvider provider, BlockWrapper wrapper) { provider.withExistingParent(wrapper.path(), PoscardsSkills.asResource(String.format("block/%s", wrapper.path()))); }

    private static void wallItemModel(ItemModelProvider provider, BlockWrapper wrapper) { provider.wallInventory(wrapper.path(), wrapper.textureLocation()); }
    ...
    private static void blockLoot(BlockLoot provider, BlockWrapper wrapper) { provider.dropSelf(wrapper.get()); }

    private static void slabLoot(BlockLoot provider, BlockWrapper wrapper) { ((PSLootTableProvider.SubProvider) provider).slab(wrapper.get()); }
