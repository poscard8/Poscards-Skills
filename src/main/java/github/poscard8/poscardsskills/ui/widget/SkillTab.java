...
public class SkillTab {
    ...
    public static ImageButton backButton(int leftPos, int topPos) {

        return new ButtonWithTooltip(leftPos, topPos, 7, 11, 212, 0, 11, SkillScreen.TEXTURE_LOCATION, 256, 256,
    ...
    public SkillTab(SkillScreen screen, SkillInstance instance, int leftPos, int topPos) {
        ...
        for (SkillMilestone milestone : milestones) {
            ...
            SkillMilestoneButton button = new SkillMilestoneButton(milestone, leftPos + 12 + column * 18, topPos + 12 + row * 18, this);
            buttons.add(button);
    ...
    public void renderBg(GuiGraphics guiGraphics) {
        ...
        for (int x = 0; x < 10; x++) {
            ...
            for (int y = 0; y < 6; y++) guiGraphics.blit(background, leftPos + 16 * x, topPos + 16 * y, first, 0, 16, 16, 16, 16);
        }
    }
    ...
    ImageButton backButton() { return backButton(leftPos + 2, topPos + 2); }
